...
class LinkRepositoryJdbcImplTest extends IntegrationEnvironment {
    ...
    List<Link> setUpLinks() {
        var links = List.of(
            linkRepositoryJdbcImpl.add(URI.create("link1"), OffsetDateTime.now()),
            linkRepositoryJdbcImpl.add(URI.create("link2"), OffsetDateTime.now()),
            linkRepositoryJdbcImpl.add(URI.create("link3"), OffsetDateTime.now()),
            linkRepositoryJdbcImpl.add(URI.create("link4"), OffsetDateTime.now())
        );
        for (int i = 0; i < 4; i++) {
            linkRepositoryJdbcImpl.update(
                new Link(
                    links.get(i).id(),
                    links.get(i).url(),
                    OffsetDateTime.now(),
    ...
    void findAllOrderedByLastCheckTime() {
        ...
        var actual1 = linkRepositoryJdbcImpl
        ...
        var actual2 = linkRepositoryJdbcImpl
            .findWithTimeSinceLastCheckLessThanGivenOrderedByLastCheckTime(
    ...
    void findById() {
        var links = setUpLinks();
        var link = linkRepositoryJdbcImpl.findById(links.get(0).id());
    ...
    void add() {
        ...
        linkRepositoryJdbcImpl.add(URI.create("link1"), OffsetDateTime.now().minusHours(1)); // should be ignored because url is the same
    ...
    @Rollback
    void update() {
        var link = linkRepositoryJdbcImpl.add(URI.create("link5"), OffsetDateTime.now());
        var updatedLink = new Link(
            link.id(), link.url(),
            OffsetDateTime.now().plusHours(1),
            OffsetDateTime.now().plusHours(1));
        linkRepositoryJdbcImpl.update(updatedLink);
        assertThat(linkRepositoryJdbcImpl.findById(link.id()).lastCheckAt())
            .isEqualToIgnoringNanos(updatedLink.lastCheckAt().withOffsetSameInstant(ZoneOffset.UTC));
        assertThat(linkRepositoryJdbcImpl.findById(link.id()).lastUpdatedAt())
    ...
    void remove() {
        ...
        linkRepositoryJdbcImpl.remove(links.get(0).id());
